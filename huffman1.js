...
class HuffmanCoder{
    ...
    encode(data){
        ...
        while(this.heap.size() > 1){
            const k1 = this.heap.extractMax();
            const k2 = this.heap.extractMax();
            const node = [k1[0]+k2[0] , [k1,k2]];
        ...
        for(let i=0 ; i<this.data.size() ; i++){
            bs = bs + this.mappings[data[i]];
        }
        let rem = (8 - bs.size()%8)%8;
        let pd = "";
        for(let i=0 ; i<rem ; i++)
           pd = pd + "0";
        bs = bs + pd;
